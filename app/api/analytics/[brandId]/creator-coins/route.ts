...
import { creatorCoinOpensTable, userTable } from "@/lib/database/db.schema";
...
const sortFields = {
  totalOpens: (dir: "asc" | "desc") =>
    sql`count(${creatorCoinOpensTable.id}) ${sql.raw(dir)}`,
  firstOpen: (dir: "asc" | "desc") =>
    sql`min(${creatorCoinOpensTable.createdAt}) ${sql.raw(dir)}`,
  lastOpen: (dir: "asc" | "desc") =>
    sql`max(${creatorCoinOpensTable.createdAt}) ${sql.raw(dir)}`,
} as const;
...
export async function GET(
  ...
  try {
    ...
    const [{ count }] = await db
      ...
      .from(creatorCoinOpensTable)
      .innerJoin(userTable, eq(creatorCoinOpensTable.openerId, userTable.id))
      .where(eq(creatorCoinOpensTable.brandId, brand.id));
    ...
    const query = db
      .select({
        ...
        totalOpens: sql<number>`count(${creatorCoinOpensTable.id})`.as(
          "totalOpens",
        ),
        firstOpen: sql<string>`min(${creatorCoinOpensTable.createdAt})`.as(
          "firstOpen",
        ),
        lastOpen: sql<string>`max(${creatorCoinOpensTable.createdAt})`.as(
          "lastOpen",
        ),
      })
      .from(creatorCoinOpensTable)
    ...
    console.error("Error in creator coins analytics:", error);
    return NextResponse.json(
