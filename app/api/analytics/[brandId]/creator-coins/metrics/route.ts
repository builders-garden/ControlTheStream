...
import { creatorCoinOpensTable } from "@/lib/database/db.schema";
...
export async function GET(
  ...
  try {
    ...
    const [metrics] = await db
      .select({
        totalOpens: sql<number>`count(${creatorCoinOpensTable.id})`,
        uniqueOpeners: sql<number>`count(distinct ${creatorCoinOpensTable.openerId})`,
      })
    ...
    return NextResponse.json({
      data: {
        ...
        uniqueOpeners: metrics.uniqueOpeners || 0,
      },
    });
  } catch (error) {
    console.error("Error in creator coins analytics metrics:", error);
    return NextResponse.json(
